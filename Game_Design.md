...
1. **以情景驱动，不以年龄驱动。** 年龄只是背景；玩家走进一程（读书、经商、上路…），在回合里做选择并看总结。
2. **选择必须改变规则状态。** 每个选项至少改变处境、关系、生活线索、属性、标签、后续事件、可用路线或结局条件之一。
...
2. 从当前等级与能力允许的天赋池中生成确定性的候选，玩家选择 2 项。天赋不再加看不见的数字，而是留下命名的光环或行囊。
3. 按权重生成家庭。家庭、天赋和轮回余裕共同决定开局带着什么光环、行囊或负累，不再分配四点属性。
...
3. 根据当前生活世界计算各领域压力：处境、关系亲疏、线索强度会互相抬高或压低，而不是只按年龄往下抽一张事件。
4. 按年龄、等级、属性、标签、天赋、历史、阶段志向、能力标签与世界状态筛选事件。
5. 避免普通随机池连续重复上一事件；权重同时受志向、传承、领域压力、跨线索耦合和近期重复惩罚影响。被冷落却仍很重的线索会把对应事件重新推到前台。
6. 普通事件立即应用；关键事件只生成并保存 `PendingLifeDecision`，等待玩家输入。
7. 事件作为世界突变生效，并可能牵动尚未出场的其他线索；随后进行死亡和终局判定。

“快进至抉择”只重复上述安全推进，并在阶段志向、关键选择或人生结束时自动停止。

### 3.5 生活世界、事件定义与回响

人生不是一棵只往下长的选择树，而是若干并行线索组成的网。出生时就会生成生活世界 `LifeWorld`：

- **处境 `facts`**：现在住在哪里、靠什么谋生、有没有成家、身体处在什么状态。
- **关系 `relations`**：家人、故人、伴侣、孩子等具体的人，带有亲疏和张力。
- **线索 `threads`**：求知、事业、家庭、远行等仍在进行的生活线，强度决定它有多常来要你的注意。

每一年被冷落的关系会自己变远，被事实撑住的线索不会无故消失；事业变重会牵动家里的节奏，远行会拉开故乡的距离，孩子出现会来抢事业的时间。事件权重看的是这张网此刻的压力，而不是只看年龄段里还剩哪些卡片。

主角不积累体魄/心智/人缘/家底四个数字。开局也不再分配点数。人带着的是命名的**光环、行囊和负累**（例如「筋骨结实」「手头宽裕」「透支」）：它们改的是以后遇见什么、同一件事里哪条结果更常发生，而不是给面板加一。

事件的正确定义是一次**世界突变**，数字变化不再是玩家看到的载荷。一个事件或选项结果可以：

- 改写处境、关系或生活线索。
- 留下、加深或散去光环、行囊、负累，并因此改变后续随机。
- 添加路线或经历标签。
- 安排一个未来年龄窗口内必定优先检查的事件。
- 因为两条线索同时吃紧而额外提高出现权重。
- 直接触发终局。

关键事件提供 2—4 个满足当前条件的选项。待决策状态会保存事件 ID、可选项 ID、随机状态、已改写事件列表，以及把玩家带到这个路口的压力说明，因此退出再进入不会刷新候选。结果历史保存 `choiceId`、`outcomeId` 和世界变化；后续事件保存 `causedByChoiceId`，用于解释“这件事源于哪次决定”。
...
flowchart TB
    ...
    KIT --> PRES
    PRES --> LIFE[lifeEngine<br/>人生状态机与事件规则]
    APP --> LIFE
    APP --> REWARD[rewardEngine<br/>评价、候选与领取]
    LIFE --> WORLD[lifeWorld<br/>处境、关系、线索与交叉压力]
...
| 路径 | 职责 |
| --- | --- |
| `assets/scripts/GameApp.ts` | 启动应用、调用 GameService、按状态调度页面，不直接拼控件 |
| `assets/scripts/app/presentation/` | 视觉配置与纯数据展示模型：把存档状态变成页面 ViewModel，可在 Node 中测试 |
| `assets/scripts/ui/` | 主题、通用控件、2.5D 矢量场景绘制与各页面渲染 |
...
| `assets/scripts/core/lifeWorld.ts` | 生活世界、事件突变、年度漂移、领域压力与跨线索权重 |
| `assets/scripts/core/lifeMarks.ts` | 光环、行囊、负累：命名印记、概率倾斜与开局授予 |
| `assets/scripts/content/markContent.ts` | 印记名录 |
...
- `currentRun`：当前或上一世，包括随机状态、年龄、光环/行囊/负累、生活世界、历史、阶段志向、待决策事件、未来安排、能力快照、命运次数、结局和结算。
...
`GameApp` 按状态调度以下运行时页面，不需要手工绑定场景控件：

- 轮回空间（轮回之门主视觉、紧凑等级经验、传承装备槽）
- 天赋选择（徽章 + 短效果，已选进入底部槽位）
- 历史地域与人物（场景缩略图与卡通肖像）
- 选路（最多三张场景预览卡）
- 情景回合（约 45% 场景区 + 资源/行动）
- 关键抉择（同场列出 2—4 个选项，选中后确认）
- 一程总结与人生结算
- 传承三选一与装配
- 错误恢复页

界面使用 Cocos 原生 `Graphics`、`Label`、`Button` 与局部 `ScrollView`，不依赖图片素材。视觉方向为明快卡通的 2.5D 场景叙事：奶油白、天空蓝、草绿与珊瑚橙，深色文字保证可读性。八类情景分别绘制庭院、书房、集市、工坊、旅途、居所、议事厅和暮年庭院；历史模式复用场景骨架，按四域调整建筑、服饰和道具。

长文案默认显示 2—3 行并可展开；光环/行囊/负累在主界面最多三枚；资源只显示真实数值，不用虚构百分比。结算提交加锁，避免连续点击重复触发。
...
当前测试覆盖 27 项，包括确定性天赋、阶段选择、关键事件暂停、选择回响、生活世界突变、跨线索耦合、关系漂移、结算与领取幂等、待事件和待奖励重载、槽位限制、能力汇总、v1 迁移、内容结构、1000 局完整人生模拟，以及展示适配层的选项完整性、资源不足禁用、预见权限、长文本展开、状态差异和八类场景映射。
