...
export function createBirthWorld(
  ...
  if (temperament?.world) {
    world = applyWorldChange(world, temperament.world, 0).world;
  }
  return world;
}
...
export function tickLifeWorld(world: LifeWorld, age: number): LifeWorld {
...
export function computeWorldPressures(world: LifeWorld, age = 0): WorldPressures {
...
export function matchesWorldCondition(
  condition: EncounterCondition | undefined,
  run: Pick<LifeRun, 'tags' | 'world'>,
...
export function formatChangePreview(change: WorldChange): string {
  ...
  return worldText.slice(0, 4).join('　');
}

export function getWorld(run: Pick<LifeRun, 'world'>): LifeWorld {
